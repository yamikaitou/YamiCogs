...
class EconomyTrickle(commands.Cog):
    ...
    async def do_rewards_for(self, guild: discord.Guild, now: datetime, data: dict):
        ...
        if data["mode"] == "blocklist":

            def mpred(m: discord.Message):
                return m.channel.id not in data["blocklist"]

            def vpred(mem: discord.Member):
                with contextlib.suppress(AttributeError):
                    return mem.voice.channel.id not in data["blocklist"] and not mem.bot

        else:

            def mpred(m: discord.Message):
                return m.channel.id in data["allowlist"]

            def vpred(mem: discord.Member):
                with contextlib.suppress(AttributeError):
                    return mem.voice.channel.id in data["allowlist"] and not mem.bot
